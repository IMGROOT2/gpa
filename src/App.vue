...
<template>
  <main class=" w-full grid h-[100vh] place-items-center">
...
const unweightedGPA = computed(() => {
  const qualityPoints = relevantCourses.value.map((el) => (el.average >= 90 ? 4 : (4-(90-el.average)/10)))
  return qualityPoints.reduce((prev, current) => prev + current, 0)/qualityPoints.length
